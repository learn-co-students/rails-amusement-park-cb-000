...
describe 'Feature Test: User Signout', :type => :feature do
  ...
  it 'has a link to sign out from the users/show page' do
    visit_signup
    user_signup
    expect(page).to have_content("Sign Out")
  end

  it 'redirects to home page after signing out' do
    visit_signup
    user_signup
    click_link("Sign Out")
    expect(current_path).to eq('/')
  ...
  it "successfully destroys session hash when 'Sign Out' is clicked" do
    visit_signup
    user_signup
    click_link("Sign Out")
  ...
  it 'has a link to sign out from the users/show page when user is an admin' do
    visit_signup
    admin_signup
    expect(page).to have_content("Sign Out")
  end

  it 'redirects to home page after admin signs out when user is an admin' do
    visit_signup
    admin_signup
    click_link("Sign Out")
    expect(current_path).to eq('/')
  ...
  it "successfully destroys session hash when 'Sign Out' is clicked as admin" do
    visit_signup
    admin_signup
    click_link("Sign Out")
    expect(page.get_rack_session).to_not include("user_id")
